...
public class BoatAutoPilot : MonoBehaviour
{
    ...
    private void Update()
    {
        Collider[] colliders = Physics.OverlapSphere(transform.position, BoatPara.Neighborhood_Radius);
    ...
    private Vector3 ComputeAcceleration(IEnumerable<BoatAutoPilot> boats)
    {
        ...
        acceleration += ComputeAlignment(boats) * BoatPara.Alignment_Amount;
        acceleration += ComputeSeparation(boats) * BoatPara.Separation_Amount;
        acceleration += ComputeCohesion(boats) * BoatPara.Cohesion_Amount;
    ...
    private void UpdateVelocity(Vector3 acceleration)
    {
        ...
        velocity = LimitMagnitude(velocity, BoatPara.Max_Speed);
    }
    ...
    private void UpdateRotation(Vector3 velocity)
    {
        ...
        transform.forward = Vector3.RotateTowards(transform.forward, velocity, Time.deltaTime * BoatPara.Steering_Speed, float.MaxValue);
    }
    ...
    private Vector3 ComputeAlignment(IEnumerable<BoatAutoPilot> boats)
    {
        ...
        var steer = Steer(velocity.normalized * BoatPara.Max_Speed);
        return steer;
    }
    ...
    private Vector3 ComputeCohesion(IEnumerable<BoatAutoPilot> boats)
    {
        ...
        var steer = Steer(direction.normalized * BoatPara.Max_Speed);
        return steer;
    }
    ...
    private Vector3 ComputeSeparation(IEnumerable<BoatAutoPilot> boats)
    {
        ...
        boats = boats.Where(boat => Vector3.Distance(transform.position, boat.transform.position) <= BoatPara.Separation_Radius);
    ...
    private Vector3 Steer(Vector3 desiredVelocity)
    {
        ...
        steer = LimitMagnitude(steer, BoatPara.Max_Force);
        return steer;
    }
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        Gizmos.DrawWireSphere(transform.position, BoatPara.Neighborhood_Radius);

        // Separation radius.
        Gizmos.color = Color.salmon;
        Gizmos.DrawWireSphere(transform.position, BoatPara.Separation_Radius);
    }
}